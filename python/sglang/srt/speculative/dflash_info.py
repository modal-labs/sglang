...
@dataclass
class DFlashDraftInputCommitLater(SpecInput):
    """Commit-later version of DFlashDraftInput.

    Key differences from DFlashDraftInput:
    - Uses DraftReqState (committed + scratch) instead of DynamicCache
    - No target_hidden carried per iteration (committed K/V already in cache)
    - ctx_lens not needed (committed_len is in DraftReqState)

    This enables:
    - Monotonic committed_len growth (paged KV compatible)
    - No rollback/crop operations
    - Stable tensor addresses (CUDA graph friendly)
    """

    # Current token to start the next DFlash block (one per request)
    verified_id: torch.Tensor

    # Per-request DraftReqState objects (commit-later cache)
    # Import type hint to avoid circular import
    req_states: List[object]  # List[DraftReqState]
    ...
    def filter_batch(self, new_indices: torch.Tensor, has_been_filtered: bool = True):
        keep_indices = new_indices.tolist()
        self.verified_id = self.verified_id[new_indices]
        self.req_states = [self.req_states[i] for i in keep_indices]

    def merge_batch(self, spec_info: "DFlashDraftInputCommitLater"):
        self.verified_id = torch.cat([self.verified_id, spec_info.verified_id], dim=0)
        self.req_states.extend(spec_info.req_states)
